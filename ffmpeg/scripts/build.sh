...
node "$package_root/scripts/verify-system-notices.mjs" "$docker_output/ffmpeg-core.map" "$docker_output/notices"
...
cp "$repository_root/LICENSE.md" "$output/notices/asbplayer-MIT.txt"
cp "$docker_output/notices/Emscripten-MIT.txt" "$output/notices/"
cp "$docker_output/notices/musl-COPYRIGHT.txt" "$output/notices/"
cp "$docker_output/notices/compiler-rt-LICENSE.txt" "$output/notices/"
cp "$docker_output/notices/libcxx-Apache-2.0.txt" "$output/notices/"
cp "$docker_output/notices/libcxxabi-LICENSE.txt" "$output/notices/"
